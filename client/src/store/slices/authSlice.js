...
export const loginWithPhone = createAsyncThunk(
  'auth/loginWithPhone',
  async ({ firebaseToken, phone }, { rejectWithValue }) => {
    try {
      const { data } = await api.post('/auth/verify-phone', { firebaseToken, phone })
      localStorage.setItem('hubdrive_token', data.token)
      return data
    } catch (err) {
...
// After Google redirects back, token is in URL ?token=xxx
export const handleGoogleCallback = createAsyncThunk(
  'auth/googleCallback',
  async (token, { rejectWithValue }) => {
    try {
      localStorage.setItem('hubdrive_token', token)
      const { data } = await api.get('/auth/me')
...
export const linkPhone = createAsyncThunk(
  ...
  async (firebaseToken, { rejectWithValue }) => {
    try {
      const { data } = await api.post('/auth/link-phone', { firebaseToken })
      return data
    } catch (err) {
      return rejectWithValue(err.response?.data?.message || 'Phone linking failed')
    }
  }
)
...
const authSlice = createSlice({
  ...
  reducers: {
    ...
    updateKycStatus: (state, action) => {
      if (state.user) state.user.kyc = { ...state.user.kyc, ...action.payload }
    },
    setToken: (state, action) => {
      state.token      = action.payload
      state.isLoggedIn = true
      localStorage.setItem('hubdrive_token', action.payload)
    },
  },
  extraReducers: (builder) => {
    ...
    builder
      ...
      .addCase(registerWithPhone.pending,   setLoading)
      .addCase(registerWithPhone.fulfilled, setAuth)
      .addCase(registerWithPhone.rejected,  setError)

      .addCase(loginWithPhone.pending,   setLoading)
      .addCase(loginWithPhone.fulfilled, setAuth)
      .addCase(loginWithPhone.rejected,  setError)

      .addCase(handleGoogleCallback.pending,   setLoading)
      ...
      .addCase(handleGoogleCallback.rejected, setError)

      .addCase(linkPhone.fulfilled, (state, action) => {
...
export const { logout, clearError, updateKycStatus, setToken } = authSlice.actions
...
export const selectIsCustomer     = (state) => state.auth.user?.role === 'customer'
export const selectKycStatus      = (state) => state.auth.user?.kyc?.status
export const selectIsPhoneVerified= (state) => state.auth.user?.isPhoneVerified
