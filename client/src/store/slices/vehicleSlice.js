...
const vehicleSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchVehicleById.fulfilled, (state, action) => {
        state.selected = action.payload?.vehicle || null
      })
      ...
      .addCase(fetchVendorVehicles.rejected, (state) => {
        state.fleetLoading = false
      })
      ...
      .addCase(updateVehicle.fulfilled, (state, action) => {
        ...
        const index = state.vendorFleet.findIndex(v => v._id === updated._id)

        if (index !== -1) {
          state.vendorFleet[index] = updated
        }
      })
      ...
      .addCase(deleteVehicle.fulfilled, (state, action) => {
        state.vendorFleet = state.vendorFleet.filter(
