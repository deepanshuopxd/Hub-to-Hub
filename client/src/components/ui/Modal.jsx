...
//       <div
//         className={`
//           relative w-full ${sizes[size]} max-h-[95vh] flex flex-col
...
const Modal = ({ isOpen, onClose, title, children, size = 'md', hideClose = false }) => {
  ...
  return (
    ...
    >
      {/* Backdrop */}
      <div className="fixed inset-0 bg-brand-black/80 backdrop-blur-sm" onClick={onClose} />
      ...
      `}>
        {/* Header */}
        {(title || !hideClose) && (
          <div className="flex items-center justify-between px-6 py-4 border-b border-white/8 sticky top-0 bg-brand-slate z-10">
            {title && (
              <span className="font-mono text-xs tracking-widest uppercase text-brand-amber">
                {title}
              </span>
